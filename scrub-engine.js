...
function mountScrollWorld(container, config) {
  ...
  // frames : on dé-verrouille, on resynchronise read() et on exécute le tick de
  // seek directement au timer (scrub dégradé mais vivant), puis on tente de
  // relancer la chaîne rAF. Coût nul quand les frames tournent normalement.
  let lastRafTick = 0;
  requestAnimationFrame(raf);
  setInterval(() => {
    if (performance.now() - lastRafTick > 300) {
      ticking = false;
      read();
      seekTick();
      requestAnimationFrame(raf);
